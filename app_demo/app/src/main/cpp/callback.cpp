...
//#define PULSE_DETECTION_MAX_RANGE_SAMPLES (48000)
...
//    rawToMagnitude(&cData[0], 2400, 600, 1800, &result[0], 300);
    float result[1200];
    rawToMagnitude(&cData[0], 2400, 600, 1800, &result[0], 1200);
//    float result[48000];
//    rawToMagnitude(&cData[0], 48000, 0, 48000, &result[0], 48000);
...
//    env->SetFloatArrayRegion(resultObj, 0, 300, result);
    jfloatArray resultObj = env->NewFloatArray(1200);
    env->SetFloatArrayRegion(resultObj, 0, 1200, result);
//    jfloatArray resultObj = env->NewFloatArray(48000);
//    env->SetFloatArrayRegion(resultObj, 0, 48000, result);
    return resultObj;
}
...
//    memcpy(&orgData[0], *(*(r->data + 0) + 1), 48000 * sizeof(float));
...
//    float result[48000];
    ...
    */
//    rawToMagnitude(&orgData[0], 48000, 0, 48000, &result[0], 48000);
    rawToMagnitude(&orgData[0], 2400, 600, 1800, &result[0], 1200);
