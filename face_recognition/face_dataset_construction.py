...
def read_file(file_name):
    ...
    sound_1d = sound_1d - raw_signal

    sound_1d_list = []
    for i in range(0, len(sound_1d), 12000):
...
def read_folder(rootpath = './dataset/', wear1 = 'open', wear2 = None,\
    ppl_list=None, use_synthesis=False):
    """
    ...
    """
    dataset = {}
    for idx, people in enumerate(sorted(ppl_list)):
        idx = str(idx)
        for place in os.listdir(os.path.join(rootpath, people)): # this inlcudes synthesis folder
            ...
            for emotion_folder in os.listdir(place_path):
                ...
                if wear2 != None:
                    ...
                    # spect_imgs2 = list(zscore(spect_imgs2, axis = None))
                if idx in dataset.keys():
                    dataset[idx].extend(spect_imgs1) # append() to separate ppl
                    dataset[idx].extend(spect_imgs2)
                else:
                    dataset[idx]=spect_imgs1
                    dataset[idx].extend(spect_imgs2)
    return dataset

class FaceSensingDataset(Dataset):
...
if __name__ == "__main__":
    ...
    dataset = FaceSensingDataset(rootpath = '../dataset/', ppl_list = ppl_list, \
                                           wear1 = wear1, wear2 = wear2, use_synthesis = use_synthesis, \
                                            local_data_name = local_data_name, saved = True, \
