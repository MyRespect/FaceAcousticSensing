...
from face_dataset_construction import FaceSensingDataset, XDomainDataset
...
def set_loader(opt):
    ...
    if opt.dataset == 'facer_sensing':
        ...
        ppl_list2 = ['mark', 'tony', 'ce']

        rootpath = '../dataset/'

        if opt.case == '1': # merge all, and split train-test
            face_data = FaceSensingDataset(rootpath = rootpath, ppl_list = ppl_list1, \
        ...
        elif opt.case == '2': # train without mask, test with mask
            train_dataset = FaceSensingDataset(rootpath = rootpath, ppl_list = ppl_list1, \
                                               wear1 = 'open', wear2 = None, use_synthesis=True, \
                                                local_data_name = 'dataset_open', saved = opt.data_saved, \
                                                    transform=TwoCropTransform(train_transform))
            val_dataset = FaceSensingDataset(rootpath = rootpath, ppl_list = ppl_list1, \
                                              wear1 = None, wear2 = 'mask', use_synthesis=False, \
                                                local_data_name = 'dataset_mask', saved = opt.data_saved, \
                                                transform=val_transform
                                                    )

        elif opt.case == '3': # train with mask, test without mask
            train_dataset = FaceSensingDataset(rootpath = rootpath, ppl_list = ppl_list1, \
                                              wear1 = None, wear2 = 'mask', use_synthesis=True, \
                                                local_data_name = 'dataset_mask', saved = opt.data_saved, \
                                                    transform=TwoCropTransform(train_transform))
            val_dataset = FaceSensingDataset(rootpath = rootpath, ppl_list = ppl_list1, \
                                              wear1 = 'open', wear2 = None, use_synthesis=False, \
            ...
            train_dataset = FaceSensingDataset(rootpath = rootpath, ppl_list = ppl_list5, \
                                               wear1 = 'open', wear2 = None, use_synthesis=True, \
                                                local_data_name = 'dataset_men', saved = False, \
                                                    transform=TwoCropTransform(train_transform))
            val_dataset = FaceSensingDataset(rootpath = rootpath, ppl_list = ppl_list5, \
                                              wear1 = 'open', wear2 = None, use_synthesis=False, \
...
def train(train_loader, model, criterion, optimizer, opt, epoch, avg_cost, lambda_weight):
    ...
    if epoch <= 5:
        ...
        lambda_weight[0, epoch] = np.exp(w_1/10)/(np.exp(w_1/10) + np.exp(w_2/10)) # overflow encountered in exp
    ...
    for idx, (samples, labels) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 2) # be careful about clip, made it hard to train
        optimizer.step()
...
def load_samples(dataset, class_num):
    center_dict = {}
    cnt = [0]*class_num # mark to stop data collection
    while sum(cnt) < class_num:
        ...
        label = label.item()
        strlabel = str(label)
        if strlabel in center_dict.keys():
            # collect 900 samples for each key to save some time
            if len(center_dict[strlabel])>900:
                cnt[label]=1
                continue
            center_dict[strlabel].append(sample)
        else:
            center_dict[strlabel] = [sample]

        if sum(cnt) == opt.n_cls:
            break
    return center_dict
...
def Kmin(feature_dict, feature_dict_mean, sample, model, use_mad=False):
    ...
    with torch.no_grad():
        ...
        if use_mad == True:
            ...
            if np.min(dist_list)> 3: # threshold for MAD
                return -1
...
def evaluation(val_loader, model, criterion, opt):
    model.eval()  # this mode effects the model performance
    ...
    with torch.no_grad():
        ...
        if top1.avg > 85: # only save data when accuraacy is greater than 85%
...
def main(opt, train_mode, use_mad):
    best_acc = 0
    ...
    train_loader, val_loader, train_dataset = set_loader(opt)
    ...
    #     model = load_model(model, model_ckpt)
    #     model = model.to(device)
    ...
    if train_mode == True:
        for i in range(opt.round):
            ...
            sample_dict = load_samples(train_dataset, opt.n_cls)
            ...
            sample_list, label_list = Kmin_adaption(sample_dict, val_loader, model, use_mad=use_mad)
            ...
            xdomain_datset = XDomainDataset(sample_list, label_list, transform=TwoCropTransform(WeightAverage()))

            x_dataset = ConcatDataset([train_dataset, xdomain_datset])
            
            train_class_vector=[]
            for _, label in x_dataset:
                train_class_vector.append(label)
            train_class_vector = torch.from_numpy(np.array(train_class_vector))
            train_sampler = StratifiedSampler(class_vector=train_class_vector, batch_size=opt.batch_size)

            train_loader = DataLoader(x_dataset, batch_size=opt.batch_size, shuffle=False, \
                                                    num_workers=opt.num_workers, pin_memory=True, sampler=train_sampler)
...
if __name__ == '__main__':
    opt = parse_option()
    opt.dataset = 'facer_sensing'
    train_mode = True
    use_mad = True
    main(opt, train_mode, use_mad)
    # python main.py --case 1 --dataset facer_sensing --epochs 100 --learning_rate 0.1 --data_saved --round 150